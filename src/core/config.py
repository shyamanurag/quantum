...
class Settings(BaseSettings):
    ...
    def database_url(self) -> str:
        ...
        # For local development, prioritize SQLite if PostgreSQL is not available
        try:
            import psycopg2
            ...
            logger.info("PostgreSQL not available, using SQLite for local development")
            return "sqlite:///./trading_system_local.db"
    ...
    BINANCE_USER_ID: Optional[str] = Field(None, description="Binance user ID")
...
# Valid Binance testnet symbols
VALID_BINANCE_SYMBOLS = [
    'BTCUSDT', 'ETHUSDT', 'BNBUSDT', 'ADAUSDT', 'DOTUSDT', 
    'LTCUSDT', 'BCHUSDT', 'XRPUSDT', 'EOSUSDT', 'TRXUSDT'
]

# Default trading symbols for strategies
DEFAULT_TRADING_SYMBOLS = ['BTCUSDT', 'ETHUSDT', 'BNBUSDT', 'ADAUSDT', 'DOTUSDT']
